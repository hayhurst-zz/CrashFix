...
struct MdmpStackFrame
{
	...
	DWORD64 m_dwOffsInLine;     // Offset from the beginning of source line
};
...
struct MdmpThread
{
	...
	ULONG32 m_dwThreadId;        // Thread ID.
    CONTEXT* m_pThreadContext; // Thread context
	ULONG32 m_uThreadContextSize;  // Size of thread context 
...
{   
	...
	ULONG32 m_uExceptionThreadContextSize;  // Size of thread context 

    std::vector<MdmpThread> m_Threads;       // The list of threads.
    std::map<DWORD, ULONG32> m_ThreadIndex;   // <thread_id, thread_entry_index> pairs
